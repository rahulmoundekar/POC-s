<section id="slider">
	<!--slider-->
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				<div id="slider-carousel" class="carousel slide"
					data-ride="carousel">
					<ol class="carousel-indicators">
						<li data-target="#slider-carousel" data-slide-to="0"
							class="active"></li>
						<li data-target="#slider-carousel" data-slide-to="1"></li>
						<li data-target="#slider-carousel" data-slide-to="2"></li>
						<li data-target="#slider-carousel" data-slide-to="3"></li>
						<li data-target="#slider-carousel" data-slide-to="4"></li>
						<li data-target="#slider-carousel" data-slide-to="5"></li>
						<li data-target="#slider-carousel" data-slide-to="6"></li>
					</ol>

					<div class="carousel-inner">
						<div class="item active">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>tackle Your To-do List</h2>
								<p>Finding time to clean your home, along with everything
									else you do, can be challenging</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/Cleaning-supplies.jpg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2></h2>
								<p>Cook it with care, well done not rare.</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/cooking.jpg"
									class="girl img-responsive" alt="" />

							</div>
						</div>

						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>House Joy</h2>
								<p></p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/home.jpg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>Plumber</h2>
								<p>Looking to Hire a Reliable Plumber in Your Area?</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/p.jpg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>Swimming</h2>
								<p>swimming</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/swimming.jpg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>Unleash Your Inner Rock Star</h2>
								<p>Music And Voice Lession</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/music.jpeg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
						<div class="item">
							<div class="col-sm-6">
								<h1>
									<span></span>CRM
								</h1>
								<h2>Lose Your Wire</h2>
								<p>Our Professionals Repair With Care</p>
							</div>
							<div class="col-sm-6">
								<img src="${pageContext.request.contextPath}/crm_assets/images/services/electrical4.jpg"
									class="girl img-responsive" alt="" />
							</div>
						</div>
					</div>

					<a href="#slider-carousel" class="left control-carousel hidden-xs"
						data-slide="prev"> <i class="fa fa-angle-left"></i>
					</a> <a href="#slider-carousel"
						class="right control-carousel hidden-xs" data-slide="next"> <i
						class="fa fa-angle-right"></i>
					</a>
				</div>

			</div>
		</div>
	</div>
</section>
<!--/slider-->
<section>
	<div class="container">
		<div class="row">
						<div class="col-sm-9 padding-right">
				<div class="features_items">
					<!--features_items-->
					<h2 class="title text-center">Best Servicers</h2>
					<div class="col-sm-4">
						<div class="product-image-wrapper">
							<div class="single-products">
								<div class="productinfo text-center">
									<img src="${pageContext.request.contextPath}/crm_assets/images/home/product1.jpg" alt="" />
									<h2>5000rs.</h2>
									<p>BabySeater</p>
									<a href="#" class="btn btn-default add-to-cart"><i
										class="fa fa-shopping-cart"></i>Add to servicecart</a>
								</div>
								<div class="product-overlay">
									<div class="overlay-content">
										<h2>5000rs.</h2>
										<p>BabySeater</p>
										<a href="#" class="btn btn-default add-to-cart"><i
											class="fa fa-shopping-cart"></i>Add to servicecart</a>
									</div>
								</div>
							</div>
							<div class="choose">
								<ul class="nav nav-pills nav-justified">
									<li><a href="#"><i class="fa fa-plus-square"></i>Add
											to wishlist</a></li>
									<li><a href="#"><i class="fa fa-plus-square"></i>Add
											to compare</a></li>
								</ul>
							</div>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="product-image-wrapper">
							<div class="single-products">
								<div class="productinfo text-center">
									<img src="${pageContext.request.contextPath}/crm_assets/images/home/product2.jpg" alt="" />
									<h2>1500rs.</h2>
									<p>Pulmber</p>
									<a href="#" class="btn btn-default add-to-cart"><i
										class="fa fa-shopping-cart"></i>Add to servicecart</a>
								</div>
								<div class="product-overlay">
									<div class="overlay-content">
										<h2>1500rs.</h2>
										<p>Pulmber</p>
										<a href="#" class="btn btn-default add-to-cart"><i
											class="fa fa-shopping-cart"></i>Add to servicecart</a>
									</div>
								</div>
							</div>
							<div class="choose">
								<ul class="nav nav-pills nav-justified">
									<li><a href="#"><i class="fa fa-plus-square"></i>Add
											to wishlist</a></li>
									<li><a href="#"><i class="fa fa-plus-square"></i>Add
											to compare</a></li>
								</ul>
							</div>
						</div>
					</div>
					
					
					
					

				</div>
				<!--features_items-->


				
			</div>
		</div>
	</div>
</section>
